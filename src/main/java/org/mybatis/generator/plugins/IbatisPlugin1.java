...
public class IbatisPlugin1 extends PluginAdapter {
	...
	public boolean sqlMapDocumentGenerated1(Document document, IntrospectedTable introspectedTable) {
		...
		//------------
		//String refid = introspectedTable.getIbatis2SqlMapNamespace() + "." + introspectedTable.getExampleWhereClauseId();
		//String refid_new = introspectedTable.getIbatis2SqlMapNamespace() + introspectedTable.getExampleWhereClauseId();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (content.contains("refid=")) {
				...
				List<Element> xes = xml.getElements();
				for(Element e : xes){
...
//									logger.info(attrs.get(j).getValue());
									...
									attrs.get(j).setValue(value.replace(".", ""));
								}
							}
						}
					}
			...
			//ncludeElement.addAttribute(new Attribute("refid", //$NON-NLS-1$
...
//							logger.info(attrs.get(j).getValue());
	...
	private void replaceRefid(XmlElement element,
		...
		element.addElement(isParameterPresentElement);
